...
**CRITICAL**: This project uses **Unity 6.3 LTS (6000.3)**. This version was released March 2026, which is beyond the LLM training data cutoff (May 2025).

Before suggesting any Unity API:
1. Read `docs/engine-reference/unity/VERSION.md` to understand version-specific risks
2. Use WebSearch to verify Unity 6.3 APIs if uncertain: `site:docs.unity3d.com Unity 6.3 [API name]`
3. Check for breaking changes between expected versions and 6000.3
4. Do NOT assume APIs from older Unity versions are unchanged in 6000.3
